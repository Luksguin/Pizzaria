...
function searchClient() {
  ...
  // Verifique se o campo de CPF foi encontrado
  if (!cpfInput) {
    console.error("Elemento 'resultCpf' não encontrado.");
    return; // Sai da função se o elemento não for encontrado
  }

  // Coleta o CPF do campo de pesquisa e garante que seja uma string
  let cpfFilter = String(cpfInput.value).trim(); // Garantimos que cpfFilter seja uma string e removemos espaços extras

  // Remove qualquer formatação (pontos, traços) do CPF
  cpfFilter = cpfFilter.replace(/[^\d]+/g, ""); // Remove tudo que não for número
  ...
  if (!validarCPF(cpfFilter)) {
    // Se o CPF não for válido, exibe o alerta e retorna
    alert("CPF inválido!");
    return;
  }

  // Filtra os clientes com base no CPF, convertendo ambos para string para garantir que a comparação funcione
  const filteredClients = clients.filter((client) => {
    const clientCpf = String(client.cpf).replace(/[^\d]+/g, ""); // Remove formatação do CPF do cliente para comparar sem formatação
    return clientCpf.includes(cpfFilter); // Usa includes para buscar parcialmente (caso necessário)
  });
